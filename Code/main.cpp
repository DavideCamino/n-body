...
void calcAcc(body& b1, body& b2) {
    ...
    std::cout<<"distance=" << distance12.magnitude() << " " << distance12<<std::endl;
...
void printLog(solarSystem& solar, int epoch) {
    std::ofstream logfile("logfile.csv", std::ios::app);
    for (int i = 0; i < solar.getSize(); i++) {
        logfile << solar.getBody(i).getPosition().toCSV();
        //logfile << ",";
        //logfile << solar.getBody(i).getVelocity().toCSV();
        //logfile << ",";
        //logfile << solar.getBody(i).getAcceleration().toCSV();
        logfile << ",";
    }
    logfile << std::endl;
}